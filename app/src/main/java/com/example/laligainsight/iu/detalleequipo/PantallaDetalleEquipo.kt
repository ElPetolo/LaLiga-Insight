...
import com.example.laligainsight.repository.RepositorioValoraciones
...
fun PantallaDetalleEquipo(
    ...
    val ratingRepository = remember { RepositorioValoraciones() }

    // Estado donde guardamos la media de valoración y el número de votos del equipo.
    var ratingSummary by remember { mutableStateOf(ResumenValoracion()) }
    ...
    LaunchedEffect(team.id) {
        ...
        try {
            ...
            val response = ClienteRetrofit.api.getTeamDetail(team.id)
            ...
            val standingsResponse = ClienteRetrofit.api.getStandings()
            ...
            try {
                val matchesResponse = ClienteRetrofit.api.getTeamMatches(team.id)
